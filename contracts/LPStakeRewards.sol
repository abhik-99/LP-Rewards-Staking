...
contract LPStakeRewards is ERC1155URIStorage, Ownable, ReentrancyGuard {
    ...
    function stake(uint256 _amount) external nonReentrant {
        ...
        require(
            stakingRegistry[user].level == 0,
            "You are not allowed to stake"
        );
    ...
    function _upgradeEligible(address _user)
        internal
        view
        returns (bool, uint8)
    {
    ...
    function unstake() external payable nonReentrant {
        ...
        if (user.level == 0 && user.time > 11 * 24 * 60 * 60) {
            if (user.amount < 10e18) {
                ...
                transferAmount = user.amount + (user.amount / 10000);
                user.amount = 0;
            } else {
                // 1.5% reward if less than 10 tokens staked
                transferAmount = user.amount + ((user.amount * 15) / 1000);
                user.amount = 0;
            }
        } else if (user.level == 1) {
            if (user.amount < 10e18) {
                // 2% reward if less than 10 tokens staked
                transferAmount = user.amount + ((user.amount * 2) / 100);
                user.amount = 0;
            } else {
                // 3% reward if less than 10 tokens staked
                transferAmount = user.amount + ((user.amount * 3) / 100);
                user.amount = 0;
            }
        } else if (user.level == 2) {
            if (user.amount < 10e18) {
                // 3.5% reward if less than 10 tokens staked
                transferAmount = user.amount + ((user.amount * 35) / 1000);
                user.amount = 0;
            } else {
                // 5% reward if less than 10 tokens staked
                transferAmount = user.amount + ((user.amount * 5) / 100);
                user.amount = 0;
            }
        } else {
            // -50% as penalty if less than 10 tokens staked
            transferAmount = user.amount - ((user.amount * 50) / 100);
            user.amount = 0;
        }
        ...
        currentRewardPool -= transferAmount;
    }
